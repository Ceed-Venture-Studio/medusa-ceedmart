...
// TipTap packages are ESM-only. The app's tsconfig targets Node16/CJS for
// the Medusa server, so tsc complains about the imports — but admin extension
// code is bundled by Vite, which handles ESM natively at runtime.
// @ts-expect-error — ESM import resolved correctly by Vite at build time
...
// Mirrors packages/admin/dashboard/src/components/common/rich-text-editor.
// That component isn't exported from a public package, so admin extensions
// can't import it — duplicated here so the Careers admin route has the same
// TipTap-backed HTML editor the dashboard uses for product descriptions.
...
export const RichTextEditor = forwardRef<Editor | null, RichTextEditorProps>(
  function RichTextEditor(
    ...
    // Sync external value changes (e.g. drawer reopens with a different row)
    // without remounting the editor — would otherwise blow away focus on
    // every render.
    useEffect(() => {
      if (!editor) return
      const next = value ?? ""
